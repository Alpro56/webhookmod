...
2. If `gradle/wrapper/gradle-wrapper.jar` is missing, regenerate it once with a local Gradle install (only necessary if you see `org.gradle.wrapper.GradleWrapperMain` errors):
   ```
   gradle wrapper
   ```
   (Any Gradle 8.x install works; this only creates the wrapper files.)
3. Build the mod with the wrapper (the provided `settings.gradle` already adds the ForgeGradle plugin repository so the plugin can resolve):
   ```
   ./gradlew build
   ```
